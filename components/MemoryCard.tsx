    ...
    <article className="memory-card">
      <div className="memory-art">
        {first?.kind === "photo" ? (
          ...
          <img className="memory-card-image" src={"/api/media/" + first.id} alt={first.caption || memory.title} />
        ) : first?.kind === "video" ? (
      ...
      <div className="memory-card-body">
        ...
        <h3>{memory.title}</h3>
        <p>{memory.story || "This memory is waiting for its story to be told."}</p>
        <div className="memory-meta">
          <span>{memory.people.length ? memory.people.join(" · ") : "People not identified yet"}</span>
          <span>Shared by {memory.contributor || "Family"}</span>
        </div>
