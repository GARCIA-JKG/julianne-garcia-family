...
export async function getArchiveHealth(): Promise<ArchiveHealth> {
  const [
    ...
    query<{
      ...
      `SELECT
        ...
        (SELECT count(*) FROM media WHERE kind = 'photo' AND archived = false AND trashed_at IS NULL)::text AS photos,
        (SELECT count(*) FROM media WHERE kind = 'video' AND archived = false AND trashed_at IS NULL)::text AS videos,
        (SELECT count(*) FROM media WHERE kind = 'audio' AND archived = false AND trashed_at IS NULL)::text AS audio,
        (SELECT count(*) FROM media WHERE trashed_at IS NOT NULL)::text AS trashed_media,
  ...
  return {
    ...
    counts: {
      ...
      trashedMedia: Number(countRow?.trashed_media ?? 0),
      voiceRecollections: Number(
